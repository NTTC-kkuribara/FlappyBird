...
//  FlappyBird
//
//  Created by ITMS NTTcom on 2021/08/29.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate /* 追加 */ {
    ...
    let appleCategory: UInt32 = 1 << 4      // 0..010000
    ...
    func setupApple() {
        ...
        // 空の中央位置を基準にしてりんごの配置場所となる中央位置を取得
        let apple_center_y = sky_center_y - appleTexture.size().height / 2
        ...
        let createAppleAnimation = SKAction.run({
            ...
            // りんごの中央位置にランダム値を足して、りんごの表示位置を決定
            // let apple_x = apple_center_x + random_x
            let apple_y = apple_center_y + random_y
            
            // りんごのノードを乗せるノードを作成
            let appleNode = SKNode()
            appleNode.position = CGPoint(x: self.frame.size.width + appleTexture.size().width / 2, y: 0)
            appleNode.zPosition = -40 // 壁よりも手前
            
            // りんごを作成
            let apple = SKSpriteNode(texture: appleTexture)
            apple.position = CGPoint(x: 0, y: apple_y)
            ...
            appleNode.addChild(apple)
            appleNode.run(appleAnimation)
            self.appleNode.addChild(appleNode)
        })
